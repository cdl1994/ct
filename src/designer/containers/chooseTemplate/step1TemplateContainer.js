            ...
            return this.props.category.templates.map((template) => {      
                return (
                    <Col md={3} sm={3} xs={6} key={template.image}>
                        <img className = "templateImg" src = {template.image} alt = {template.image} onClick = {() => this.props.actions.selectTemplate(template.image_blank)}></img>
                    </Col>
                );  
            });
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.renderList()}
                <img src="/image/data/custom-tobacco/change-selection.png" onClick = {() => this.props.actions.selectChange()}/>
            </div>
        );
